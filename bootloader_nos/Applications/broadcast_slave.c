...
void modbus_broadcast_process(void)
{
    static int __rx_state = RS485_RECV_STATE_START;
    static uint8_t *__used_buf_ptr = NULL;
    static int __remain_length = 0;
    static int __total_len = 0;
    static int __is_reset = 0;
    ...
    case 2: {
        int read_len = rs485_receive(ctx->read_buf + __remain_length, ctx->read_bufsz - __remain_length,
                                     1000, 50, &__rx_state);
        if (__rx_state == RS485_RECV_STATE_FINISH) {
            __total_len = read_len + __remain_length;

            __is_reset = 0;
            if (__total_len && read_len == 0)
                __is_reset = 1;
    ...
    case 3: {
        if (__total_len > 0) {
            int frame_length = 0;
            int rc = agile_modbus_slave_handle(ctx, __total_len, 1, slave_callback, NULL, &frame_length);
            if (rc >= 0) {
                ctx->read_buf = ctx->read_buf + frame_length;
                ctx->read_bufsz = ctx->read_bufsz - frame_length;

                __remain_length = __total_len - frame_length;
                __total_len = __remain_length;
            } else {
                if (__total_len > 1600 || __is_reset) {
                    ctx->read_buf++;
                    ctx->read_bufsz--;
                    __total_len--;
                    break;
                ...
                if (__used_buf_ptr == gbl_attr.ctx_read_buf1) {
                    memcpy(gbl_attr.ctx_read_buf2, ctx->read_buf, __total_len);

                    ctx->read_buf = gbl_attr.ctx_read_buf2;
                    ctx->read_bufsz = sizeof(gbl_attr.ctx_read_buf2);
                    __used_buf_ptr = gbl_attr.ctx_read_buf2;
                } else {
                    memcpy(gbl_attr.ctx_read_buf1, ctx->read_buf, __total_len);

                    ctx->read_buf = gbl_attr.ctx_read_buf1;
                    ctx->read_bufsz = sizeof(gbl_attr.ctx_read_buf1);
                    __used_buf_ptr = gbl_attr.ctx_read_buf1;
                }

                __remain_length = __total_len;
